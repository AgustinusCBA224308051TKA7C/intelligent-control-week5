...
if not cap.isOpened():
    print("Error: Cannot open webcam. Check if it's in use or try a different index.")
    exit()
...
with mp_hands.Hands(
    ...
    # Open CSV once, outside the loop
        ...
        while cap.isOpened():
            ...
            if results.multi_hand_landmarks and results.multi_handedness:
                for idx, (hand_landmarks, handedness) in enumerate(
                    ...
                    # Simpan ke CSV (joint_id = 0–20) - batch write, no reopen
                    for j, lm in enumerate(hand_landmarks.landmark):
                        csv_writer.writerow([frame_count, label, j, lm.x, lm.y, lm.z])
                        f.flush()  # Optional: Flush to disk periodically if needed
...
print(f"Data saved to {csv_file}. Total frames processed: {frame_count}")
